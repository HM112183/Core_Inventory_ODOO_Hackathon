...
async function startServer() {
  ...
  app.get("/api/dashboard", authenticateToken, (req, res) => {
    ...
    const lowStockItems = db.prepare(`
      ...
      JOIN (SELECT productId, SUM(quantity) as total FROM stock GROUP BY productId) s ON p.id = s.productId
      WHERE s.total <= p.reorderLevel
    `).get() as any;
  ...
  // Seed initial warehouse if empty
